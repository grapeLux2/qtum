...
#include <qt/luxversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    luxVersionChecker = new LuxVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history"), tr("&Transactions"), this);//LUX_LINE
    ...
    LRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/lrctoken"), tr("&QRC Tokens"), this);
    LRCTokenAction->setStatusTip(tr("QRC Tokens (send, receive or add Tokens in list)"));
    LRCTokenAction->setToolTip(LRCTokenAction->statusTip());
    LRCTokenAction->setCheckable(true);
    LRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(LRCTokenAction);
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Lux command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(LRCTokenAction, tokenActions);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && luxVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(LUX_RELEASES, LUX_RELEASES);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    LRCTokenAction->setEnabled(enabled);
...
void BitcoinGUI::gotoSendTokenPage()
{
    ...
    LRCTokenAction->setChecked(true);
...
{
    QString strTitle = tr("Lux"); // default title
