...
LuxVersionChecker::LuxVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

LuxVersionChecker::~LuxVersionChecker()
{

}

bool LuxVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> LuxVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(LUX_RELEASES)));
    QEventLoop event;
...
Version LuxVersionChecker::getMaxReleaseVersion()
{
