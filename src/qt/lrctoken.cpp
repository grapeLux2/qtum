...
#include <qt/forms/ui_lrctoken.h>
...
LRCToken::LRCToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LRCToken),
    m_model(0),
    m_clientModel(0),
    ...
    m_tokenTransactionView(0)
{
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &LRCToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &LRCToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &LRCToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &LRCToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &LRCToken::removeToken);

    connect(ui->tokensList, &QListView::clicked, this, &LRCToken::on_currentTokenChanged);
    connect(ui->tokensList, &QListView::customContextMenuRequested, this, &LRCToken::contextualMenu);
...
void LRCToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    if(m_model && m_model->getTokenItemModel())
    {
        ...
        connect(ui->tokensList->selectionModel(), &QItemSelectionModel::currentChanged, this, &LRCToken::on_currentChanged);

        // Set current token
        connect(m_tokenModel, &QAbstractItemModel::dataChanged, this, &LRCToken::on_dataChanged);
        connect(m_tokenModel, &QAbstractItemModel::rowsInserted, this, &LRCToken::on_rowsInserted);
...
void LRCToken::setClientModel(ClientModel *_clientModel)
{
...
void LRCToken::on_goToSendTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(0);
}

void LRCToken::on_goToReceiveTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(1);
}

void LRCToken::on_goToAddTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(2);
}

void LRCToken::on_currentTokenChanged(QModelIndex index)
{
    if(m_tokenModel)
    {
        if(index.isValid())
...
void LRCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void LRCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void LRCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void LRCToken::contextualMenu(const QPoint &point)
{
