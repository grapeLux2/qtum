...
inline void initState(){
    ...
    const std::string dirLux = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<LuxState>(new LuxState(dev::u256(0), LuxState::openDB(dirLux, hashDB, dev::WithExisting::Trust), dirLux + "/luxDB", dev::eth::BaseState::Empty));
...
inline dev::Address createLuxAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline LuxTransaction createLuxTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    LuxTransaction txEth;
    if(recipient == dev::Address()){
        txEth = LuxTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = LuxTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<LuxTransaction> txs){
