...
void checkResult(bool isCreation, std::vector<LuxTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    LuxTxConverter converter(transaction, NULL);
    ExtractLuxTX luxTx;
    BOOST_CHECK(converter.extractionLuxTransactions(luxTx));
    std::vector<LuxTransaction> result = luxTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(luxtxconverter_tests, TestingSetup)
